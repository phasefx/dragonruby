/* Modified by Jason Etheridge for use in 3D Baffle, January 30, 2001 */
...
/* ---Modifications.  Exposing some internals.  1/30/01 */
#include "global.h"
...
/* Modification, originally #define INIT_DIST      8.0   */
#define INIT_DIST      13.4
...
GLfloat Ex = INIT_EX,		/* flying parameters */
 Ey = INIT_EY, Ez = INIT_EZ, EyeMove = INIT_MOVE, EyeDist = INIT_DIST,	/* polar params */
 AzSpin = INIT_AZ_SPIN, ElSpin = INIT_EL_SPIN, EyeAz = INIT_POLAR_AZ,	/* used by both */
 EyeEl = INIT_POLAR_EL;

int agvMoving;			/* Currently moving?  */

int downx, downy,		/* for tracking mouse position */
 lastx, lasty, downb = -1;	/* and button status */
...
static void ncrossprod(float v1[], float v2[], float cp[]);
...
/* ---Modifications.  Keeping EyeAz between 0 and 360.  2/6/01 */
	if (EyeAz > 360)
	    EyeAz -= 360;
	else if (EyeAz < 0)
	    EyeAz += 360;
...
/* ---Modifications.  Doing Right mouse button  2/2/01 */
	    break;

	case GLUT_RIGHT_BUTTON:
	    if (MY_CLICK == 1)
		break;
	    MY_CLICK = 1;	/* processHits tests for this */
	    FROM_AGV = 1;
	    my_display();
	    FROM_AGV = -1;
	    break;
...
/* ---Modifications.  Don't want letters to hilite while dragging  2/2/01 */
    s_x = 9999;
    s_y = 9999;
    ...
    case GLUT_LEFT_BUTTON:
	EyeEl =
	    downEl + EL_SENS * ((MoveMode == FLYING) ? -deltay : deltay);
	ConstrainEl();
	...
	Ex = downEx -
	    E_SENS * deltay * sin(TORAD(EyeAz)) * cos(TORAD(EyeEl));
	Ey = downEy - E_SENS * deltay * sin(TORAD(EyeEl));
	Ez = downEz +
	    E_SENS * deltay * cos(TORAD(EyeAz)) * cos(TORAD(EyeEl));
	break;
    }
    glutPostRedisplay();
}
